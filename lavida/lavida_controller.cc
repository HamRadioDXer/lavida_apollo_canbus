...
#include "modules/canbus/vehicle/lavida/lavida_controller.h"

#include "cyber/common/log.h"
#include "cyber/time/time.h"
#include "modules/canbus/vehicle/lavida/lavida_message_manager.h"
...
ErrorCode LavidaController::Init(
    const VehicleParameter& params,
    CanSender<::apollo::canbus::ChassisDetail>* const can_sender,
    MessageManager<::apollo::canbus::ChassisDetail>* const message_manager) {
  if (is_initialized_) {
    AINFO << "LavidaController has already been initiated.";
  ...
  // Sender part
  accel_cmd_062_ = dynamic_cast<Accelcmd062*>(
      message_manager_->GetMutableProtocolDataById(Accelcmd062::ID));
  if (accel_cmd_062_ == nullptr) {
    AERROR << "Accelcmd062 does not exist in the LavidaMessalexusanager!";
    return ErrorCode::CANBUS_ERROR;
  }

  brake_cmd_060_ = dynamic_cast<Brakecmd060*>(
      message_manager_->GetMutableProtocolDataById(Brakecmd060::ID));
  if (brake_cmd_060_ == nullptr) {
    AERROR << "Brakecmd060 does not exist in the LavidaMessalavidaanager!";
    return ErrorCode::CANBUS_ERROR;
  }

  shift_cmd_066_ = dynamic_cast<Shiftcmd066*>(
      message_manager_->GetMutableProtocolDataById(Shiftcmd066::ID));
  if (shift_cmd_066_ == nullptr) {
    AERROR << "Shiftcmd066 does not exist in the LavidaMessalavidmanager!";
    return ErrorCode::CANBUS_ERROR;
  }
...
//   }
  ...
  steering_cmd_064_ = dynamic_cast<Steercommand064*>(
      message_manager_->GetMutableProtocolDataById(Steercommand064::ID));
  if (steering_cmd_064_ == nullptr) {
    AERROR << "Steeringcmd12c does not exist in the LexusMessageManager!";
    return ErrorCode::CANBUS_ERROR;
  }

  can_sender_->AddMessage(Accelcmd062::ID, accel_cmd_062_, false);
  can_sender_->AddMessage(Brakecmd060::ID, brake_cmd_060_, false);
  can_sender_->AddMessage(Shiftcmd066::ID, shift_cmd_066_, false);
  can_sender_->AddMessage(Steercommand064::ID, steering_cmd_064_, false);
...
LavidaController::~LavidaController() {}

bool LavidaController::Start() {
  if (!is_initialized_) {
...
Chassis LavidaController::chassis() {
  chassis_.Clear();
  ...
  // 5
  if (chassis_detail.lavida.has_vehicle_speed_rpt_400() &&
